...
public class BVManagedUIContent_CharacterEncoding_Test {
	...
	private static final String CLOUD_KEY_JAPANESE = 
	...
	public void testDefaultUTF_8_Review() {
		...
		bvParameters.setBaseURI("http://www.example.com/store/products/"
				+ "reviews");
		bvParameters.setPageURI("http://www.example.com/store/products/"
				+ "data-gen-696yl2lg1kurmqxn88fqif5y2/?utm_campaign=bazaarvoice"
				+ "&utm_medium=SearchVoice&utm_source=RatingsAndReviews&utm_"
				+ "content=Default&bvrrp=Main_Site-en_GB/reviews/product/1/"
				+ "0101707101.htm&bvreveal=debug");
		...
		assertNotNull(content, "There should be content to proceed further "
				+ "assertion!!");
		assertFalse(content.contains("HTTP 403 Forbidden"), "There should be "
				+ "valid content");
...
//		System.out.println(content);
	...
	private BVUIContent getJIS_EcodingConfiguration() {
		...
		_bvConfiguration.addProperty(BVClientConfig.SEO_SDK_ENABLED, "true");
		_bvConfiguration.addProperty(BVClientConfig.STAGING, "true");
		_bvConfiguration.addProperty(BVClientConfig.EXECUTION_TIMEOUT, 
				"60000");
